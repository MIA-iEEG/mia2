function [thresh_dur, dur_array, capt] = mia_get_bootthresh(nboot, zs, threshp) 
% ***********************************************************************
%  [] = mia_get_bootthresh 
...
% zs :      zscored amplitudes (nsens x ntimes x ntrials)
% threshp : threshold on p
% ========================================================================
...
% 
% 2014/10/28 : ASD+ CGB : Creation
% 
% 
ntrials = size(zs,3) ; 
dur_array = zeros(nboot,1);

% Create progress bar
hwait_boot = waitbar(0,sprintf('Computing bootstrap %d/%d...',0, nboot),'Name','Statistics') ;
...
for ii=1:nboot 
    ...
    % Gets the significant segments (seg in duration)
    [seg,onsets] = mia_get_significant_segments(h);

    if isempty(seg) ; seg = 0 ; end
        
    % Keep max for building distribution
    [dur_array(ii), idx] = max(seg) ; 
    
    % Get sensor on which the max segment is found
    [r, c] = ind2sub(size(h'), 1:length(h(:)));
    if isempty(onsets) ; capt(ii)=0; else capt(ii) = c(onsets(idx)); end
    
end 

% Threshold at 95%
thresh_dur = quantile(dur_array,0.95,[],'R-5'); % jcm, use same default as Matlab
...
function [q,N] = quantile(X,p,dim,method)
